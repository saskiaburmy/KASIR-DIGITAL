...
const createOrderData = ({ customerName, customerPhone, customerTable, paymentMethod, orderType, notes, total, cart, promoCode, discount, tax, service, subtotal }) => {
  const timestamp = Date.now();
  return {
    customerName, customerPhone, customerTable, paymentMethod, orderType, notes,
    subtotal, total, promoCode, discount, tax, service, orders: cart, timestamp,
...
const initialMenuItems = [
  ...
  { id: 10, name: "Hazelnut Coffee", category: "Drinks", image: hazelnut, price: 22000, sold: 30, status: "Tersedia", hasSizes: true, sizes: [{ label: "S", price: 22000 }, { label: "M", price: 27000 }, { label: "L", price: 32000 }] },
  { id: 11, name: "Matcha Latte", category: "Drinks", image: matcha, price: 24000, sold: 26, status: "Tersedia", hasSizes: true, sizes: [{ label: "S", price: 24000 }, { label: "M", price: 29000 }, { label: "L", price: 34000 }] },
  { id: 12, name: "Caffe Latte", category: "Drinks", image: caffe, price: 21000, sold: 18, status: "Tersedia", hasSizes: true, sizes: [{ label: "S", price: 21000 }, { label: "M", price: 26000 }, { label: "L", price: 31000 }] },
  { id: 13, name: "Lemon Tea", category: "Drinks", image: lemon, price: 18000, sold: 14, status: "Tersedia", hasSizes: true, sizes: [{ label: "S", price: 18000 }, { label: "M", price: 22000 }, { label: "L", price: 26000 }] },
];

// ── TOP SELLING CHART COMPONENT ──────────────────────────────────────────────
function TopSellingChart({ menuItems, history }) {
  // Merge base sold counts + actual transaction history
  const salesMap = {};
      ...
      <div style={{
        ...
        <div style={{ padding: "0 24px 24px" }}>
          ...
          <div style={{
            ...
            <span style={{
              fontSize: 22, fontWeight: "800", color: C.crimson,
              letterSpacing: 0.5,
            }}>Rp {currency(order.total)}</span>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <button onClick={onPrintPhysical} style={{
              ...btn.primary, width: "100%", padding: 13, fontSize: 13,
            ...
            <button onClick={onClose} style={{
              ...btn.ghost, width: "100%", padding: 13, fontSize: 13, fontFamily: "inherit",
            }}>✕ Tutup Struk</button>
          </div>
        </div>
      </div>
...
function App() {
  ...
  const [menuForm, setMenuForm] = useState({
    id: null, name: "", price: "", category: "Food", image: "", status: "Tersedia", hasSizes: false,
 ...
 // JIKA USER BELUM LOGIN (userRole masih kosong "")
  if (!userRole === "") {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100vh", backgroundColor: "#f4f4f4" }}>
        <div style={{ padding: "40px", backgroundColor: "#fff", borderRadius: "12px", boxShadow: "0 4px 12px rgba(0,0,0,0.1)", textAlign: "center" }}>
          <h2 style={{ marginBottom: "20px", color: "#333", fontFamily: "inherit" }}>Login Kasir Sejuta Tawa</h2>
          ...
          <button 
            ...
            style={{ padding: "10px 20px", backgroundColor: "#A04040", color: "#fff", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: "bold" }}
          >
            Masuk
          </button>
        </div>
  ...
  const handleSizeSelect = (item, size) => setSelectedSizes(prev => ({ ...prev, [item.name]: size }));
  ...
  const isGmail = (value) => typeof value === "string" && /^[^\s@]+@gmail\.com$/i.test(value);
  ...
  const updateStockAfterOrder = (cartItems) => {
    const updated = stockItems.map(stock => {
      const itemUsage = cartItems.reduce((sum, cartItem) => {
      ...
      return { ...stock, qty: Math.max(0, stock.qty - itemUsage) };
    });
    setStockItems(updated);
  };

  const toggleTableStatus = (tableName) => {
    setTables(prev => prev.map(table => table.name === tableName ? { ...table, status: table.status === "Kosong" ? "Terisi" : "Kosong" } : table));
  };

  const startEditMenu = (item) => {
    setEditingMenuId(item.id);
    setMenuForm({ ...item, price: item.price.toString(), sizes: item.sizes || [{ label: "S", price: 0 }, { label: "M", price: 0 }, { label: "L", price: 0 }] });
  };

  const saveMenuUpdate = () => {
    setMenuItems(prev => prev.map(item => item.id === editingMenuId ? { ...item, ...menuForm, price: Number(menuForm.price) } : item));
  ...
  const updateStockQty = (id, value) => {
    setStockItems(prev => prev.map(stock => stock.id === id ? { ...stock, qty: Math.max(0, Number(value)) } : stock));
  };

  const voidOrder = (index) => {
    setHistory(prev => prev.map((item, idx) => idx === index ? { ...item, status: "Dibatalkan" } : item));
  ...
  const printReceipt = (order) => {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Struk</title>
      <style>body{font-family:Arial,sans-serif;padding:10px;color:#000}.paper{width:320px;margin:0 auto;border-top:2px dashed #333;border-bottom:2px dashed #333;padding:12px}.items{font-family:monospace;margin-top:12px}.total{display:flex;justify-content:space-between;font-weight:800;margin-top:12px;font-size:16px;padding-top:10px;border-top:1px dashed #333}</style>
      </head><body>
      <div class="paper">
  ...
  const addCustomizedToCart = () => {
    ...
    const custom = {
      ...
      modifiers: { temp: modifierItem.category === "Drinks" ? modifierState.temp : undefined, sweetness: modifierItem.category === "Drinks" ? modifierState.sweetness : undefined, addOns: modifierState.addOns, notes: modifierState.notes },
    };
  ...
  const handleCheckout = () => {
    ...
    const orderData = createOrderData({ customerName, customerPhone, customerTable, paymentMethod, orderType, notes, subtotal, total: finalTotal, cart, promoCode: promoCode || "", discount, tax, service });
  ...
  const handleTambahMenu = () => {
    ...
    setMenuForm({ id: null, name: "", price: "", category: "Food", image: "", status: "Tersedia", hasSizes: false, sizes: [{ label: "S", price: 0 }, { label: "M", price: 0 }, { label: "L", price: 0 }] });
  ...
  if (loading) {
    return (
      <div style={{ background: `linear-gradient(135deg, ${C.crimsonDark} 0%, ${C.crimson} 60%, ${C.charcoal} 100%)`, color: C.cream, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", height: "100vh", fontFamily: "Georgia, serif" }}>
        <div style={{ width: 130, height: 130, borderRadius: "50%", border: `3px solid ${C.gold}`, padding: 4, boxShadow: `0 0 40px rgba(201,169,110,0.4)`, animation: "spin 3s linear infinite" }}>
  ...
  if (!loggedIn) {
    return (
      <div style={{ background: `linear-gradient(145deg, ${C.crimsonDark} 0%, ${C.charcoal} 50%, ${C.crimson} 100%)`, display: "flex", justifyContent: "center", alignItems: "center", height: "100vh", fontFamily: "Georgia, serif" }}>
        <div style={{ background: C.cream, padding: 44, borderRadius: 20, width: 380, textAlign: "center", boxShadow: `0 20px 60px rgba(74,15,15,0.5)`, border: `1px solid ${C.goldLight}` }}>
          <div style={{ width: 88, height: 88, borderRadius: "50%", margin: "0 auto 16px", border: `3px solid ${C.gold}`, padding: 3, boxShadow: `0 4px 20px rgba(123,28,28,0.3)` }}>
            <img src={logo} alt="" style={{ width: "100%", height: "100%", borderRadius: "50%", objectFit: "cover" }} />
          </div>
          <h2 style={{ color: C.crimsonDark, margin: "0 0 4px", fontSize: 22, fontWeight: 700, letterSpacing: 1 }}>{isRegisterMode ? "Daftar Akun Baru" : "Sejuta Tawa POS"}</h2>
          <p style={{ color: C.muted, fontSize: 13, marginBottom: 28, letterSpacing: 0.3 }}>{isRegisterMode ? "Buat akun kasir baru" : "Silakan login untuk masuk sistem kasir"}</p>
          <input type="email" placeholder="Email Gmail" value={loginUsername} onChange={e => setLoginUsername(e.target.value)} style={{ ...inputStyle, width: "100%", marginBottom: 12, boxSizing: "border-box" }} />
          <input type="password" placeholder="Password" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} style={{ ...inputStyle, width: "100%", marginBottom: 24, boxSizing: "border-box" }} />
          ...
          <p style={{ marginTop: 18, color: C.crimson, fontSize: 12, cursor: "pointer", letterSpacing: 0.5 }} onClick={() => setIsRegisterMode(!isRegisterMode)}>
  ...
  // ── MAIN APP ───────────────────────────────────────────────────────────────
  return (
    <div style={{ display: "flex", height: "100vh", background: C.parchment, fontFamily: "Georgia, 'Times New Roman', serif", overflow: "hidden" }}>
      <audio ref={audioRef} autoPlay loop src={music} />

      {/* SIDEBAR */}
      <div style={{ width: 248, background: `linear-gradient(180deg, ${C.crimsonDark} 0%, ${C.charcoal} 100%)`, color: C.mutedLight, display: "flex", flexDirection: "column", padding: 20, borderRight: `1px solid ${C.crimsonDark}` }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 28, paddingBottom: 18, borderBottom: `1px solid rgba(201,169,110,0.2)` }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {[
              ...
              <button key={nav.label} onClick={nav.action} style={{
                ...
                background: nav.active ? `linear-gradient(135deg, rgba(123,28,28,0.6) 0%, rgba(201,169,110,0.15) 100%)` : "transparent",
        ...
        <button onClick={() => setUserRole("")} style={{ ...btn.danger, padding: "11px 0", fontSize: 13, marginTop: "auto", letterSpacing: 0.5, fontFamily: "inherit" }}
          onMouseEnter={e => e.currentTarget.style.transform = "translateY(-2px)"}
          onMouseLeave={e => e.currentTarget.style.transform = "translateY(0)"}
        ...
        {adminMode ? (
          <div style={{ width: "100%" }}>
              ...
              <div style={{ display: "flex", gap: 6, background: C.creamDark, padding: 5, borderRadius: 12, border: `1px solid ${C.goldLight}` }}>
                {["Ringkasan", "Kelola Menu", "Kelola Meja", "Stok", "Transaksi"].map(t => (
                  <button key={t} onClick={() => setAdminTab(t)} style={{
                    ...
                    background: adminTab === t ? `linear-gradient(135deg, ${C.crimson}, ${C.crimsonDark})` : "transparent",
              ...
              <div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 4 }}>
                  {[
                    ...
                    <div key={card.label} style={{ background: C.white, padding: 22, borderRadius: 14, border: `1px solid ${C.goldLight}`, boxShadow: `0 3px 12px rgba(123,28,28,0.06)`, borderLeft: `4px solid ${C.crimson}` }}>
            ...
            {adminTab === "Kelola Menu" && (
              <div>
                <div style={{ background: C.white, borderRadius: 14, padding: 20, border: `1px solid ${C.goldLight}`, marginBottom: 20 }}>
                  {menuItems.map(item => (
                    <div key={item.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 0", borderBottom: `1px solid ${C.creamDark}` }}>
                  ...
                  {editingMenuId && (
                    <div style={{ marginTop: 20, padding: 16, background: C.cream, borderRadius: 10, border: `1px solid ${C.goldLight}` }}>
                      ...
                      <input type="number" value={menuForm.price} onChange={e => setMenuForm({ ...menuForm, price: e.target.value })} style={{ ...inputStyle, marginRight: 8 }} />
                      <button onClick={saveMenuUpdate} style={{ ...btn.success, padding: "9px 18px", fontSize: 13, fontFamily: "inherit" }}>✓ Simpan</button>
                    </div>
                  )}
                </div>
                <div style={{ background: C.white, borderRadius: 14, padding: 22, border: `2px dashed ${C.crimson}`, boxShadow: `0 4px 16px rgba(123,28,28,0.06)` }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      <input type="file" accept="image/*" onChange={e => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = ev => setMenuForm({ ...menuForm, image: ev.target.result }); reader.readAsDataURL(file); }} style={{ ...inputStyle, cursor: "pointer", fontSize: 12 }} />
                      {menuForm.image && <img src={menuForm.image} alt="preview" style={{ width: 60, height: 60, objectFit: "cover", borderRadius: 6, border: `1px solid ${C.goldLight}` }} />}
                    </div>
                  </div>
                  <button onClick={handleTambahMenu} style={{ ...btn.primary, padding: "10px 26px", fontSize: 13, letterSpacing: 0.5, fontFamily: "inherit" }}
                    onMouseEnter={e => e.currentTarget.style.transform = "translateY(-2px)"}
                    onMouseLeave={e => e.currentTarget.style.transform = "translateY(0)"}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 12 }}>
                {tables.map(t => (
                  <button key={t.name} onClick={() => toggleTableStatus(t.name)} style={{
                    ...
                    color: t.status === "Terisi" ? C.red : C.green, fontWeight: "700", fontSize: 13,
                    boxShadow: "0 2px 8px rgba(0,0,0,0.06)", transition: "all 0.2s ease",
                  }}
                    onMouseEnter={e => e.currentTarget.style.transform = "scale(1.04)"}
                    onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}
                  >{t.name}<br /><small style={{ fontWeight: 400 }}>{t.status}</small></button>
                ))}
              </div>
            )}
            ...
            {adminTab === "Stok" && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
                {stockItems.map(s => (
                  <div key={s.id} style={{ background: C.white, padding: 18, borderRadius: 12, border: `1px solid ${C.goldLight}`, borderLeft: `4px solid ${s.qty <= s.threshold ? C.red : C.crimson}`, boxShadow: "0 2px 8px rgba(0,0,0,0.04)" }}>
                    <h4 style={{ color: C.charcoal, margin: "0 0 10px 0" }}>{s.label}</h4>
                    <input type="number" value={s.qty} onChange={e => updateStockQty(s.id, e.target.value)} style={{ ...inputStyle, width: "100%", boxSizing: "border-box" }} />
                    {s.qty <= s.threshold && <small style={{ color: C.red, marginTop: 4, display: "block" }}>⚠ Stok hampir habis</small>}
                  </div>
                ))}
              </div>
            )}
            ...
            {adminTab === "Transaksi" && (
              <div style={{ background: C.white, padding: 16, borderRadius: 14, border: `1px solid ${C.goldLight}` }}>
                {history.length === 0 && <p style={{ color: C.muted, textAlign: "center", padding: 20 }}>Belum ada transaksi</p>}
                {history.map((h, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "13px 0", borderBottom: `1px solid ${C.creamDark}` }}>
                    <div>
                    ...
                    <div style={{ textAlign: "right" }}>
                      <strong style={{ color: h.status === "Dibatalkan" ? C.red : C.crimsonDark }}>Rp {h.total.toLocaleString()}</strong>
                      <br /><button onClick={() => voidOrder(i)} style={{ fontSize: 11, color: C.red, background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", fontWeight: 600 }}>Void</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
              {tables.map(table => (
                <button key={table.name}
                  onClick={() => { if (table.status === "Kosong") { setCustomerTable(table.name); setUserMenuSelection("Kasir Utama"); sendToast(`Meja ${table.name} dipilih`); } else { sendToast("Meja ini sedang terisi", "warning"); } }}
                  style={{
                    padding: 24, borderRadius: 14, cursor: "pointer", fontFamily: "inherit",
                    border: customerTable === table.name ? `2px solid ${C.crimson}` : table.status === "Terisi" ? `1.5px solid #FCA5A5` : `1.5px solid ${C.goldLight}`,
                    background: customerTable === table.name ? `linear-gradient(135deg, rgba(123,28,28,0.08), rgba(201,169,110,0.12))` : table.status === "Terisi" ? "#FEE2E2" : C.white,
                    color: table.status === "Terisi" ? C.red : C.charcoal, fontWeight: "700", textAlign: "center",
          ...
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
              ...
              <input type="text" placeholder="🔍 Cari menu makanan/minuman..." value={search} onChange={e => setSearch(e.target.value)} style={{ ...inputStyle, width: 270 }} />
            </div>
            <div style={{ display: "flex", gap: 8, marginBottom: 20, borderBottom: `2px solid ${C.goldLight}`, paddingBottom: 14 }}>
              {["All", "Food", "Drinks", "Snack", "Dessert"].map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)} style={{
                  ...
                  background: activeCategory === cat ? `linear-gradient(135deg, ${C.crimson}, ${C.crimsonDark})` : C.white,
                  color: activeCategory === cat ? C.white : C.muted,
                  boxShadow: activeCategory === cat ? `0 4px 12px rgba(123,28,28,0.3)` : `0 1px 4px rgba(0,0,0,0.06)`,
                  border: activeCategory === cat ? "none" : `1px solid ${C.goldLight}`,
                  transition: "all 0.2s ease", transform: activeCategory === cat ? "translateY(-1px)" : "translateY(0)",
                }}>{cat}</button>
              ))}
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(196px, 1fr))", gap: 16 }}>
              {filteredMenu.map(item => (
                <div key={item.id} style={{ background: C.white, borderRadius: 14, border: `1px solid ${C.goldLight}`, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 2px 10px rgba(123,28,28,0.06)", transition: "transform 0.2s ease, box-shadow 0.2s ease" }}
                ...
                >
                  <div style={{ position: "relative" }}>
                    ...
                    <span style={{ position: "absolute", top: 8, left: 8, background: `rgba(74,15,15,0.85)`, color: C.goldLight, padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 700, letterSpacing: 0.5 }}>{item.category}</span>
                  </div>
                  <div style={{ padding: 13, display: "flex", flexDirection: "column", flex: 1 }}>
                      ...
                      <div style={{ display: "flex", gap: 4, marginBottom: 10 }}>
                        {item.sizes.map(sz => (
                          <button key={sz.label} onClick={() => handleSizeSelect(item, sz)} style={{
                            ...
                            borderRadius: 6,
                            background: selectedSizes[item.name]?.label === sz.label ? `linear-gradient(135deg, ${C.crimson}, ${C.crimsonDark})` : C.cream,
                    ...
                    <div style={{ display: "flex", gap: 6, marginTop: "auto" }}>
                      <button onClick={() => addToCart(item)} style={{
                        ...
                        onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}
                      >+ Tambah</button>
                      <button onClick={() => openModifiers(item)} style={{
        ...
        <div style={{ width: 368, background: C.white, borderLeft: `1px solid ${C.goldLight}`, display: "flex", flexDirection: "column", height: "100vh" }}>
          ...
          <div style={{ flex: 1, padding: "16px 18px", overflowY: "auto", display: "flex", flexDirection: "column", gap: 10 }}>
            {cart.length === 0 ? (
              ...
              cart.map((item, idx) => (
                <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "start", paddingBottom: 10, borderBottom: `1px solid ${C.creamDark}`, fontSize: 13 }}>
                  <div>
          ...
          <div style={{ padding: 18, background: C.cream, borderTop: `1px solid ${C.goldLight}` }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, fontWeight: "700", color: C.charcoal, marginTop: 10, paddingTop: 10, borderTop: `1.5px dashed ${C.goldLight}` }}>
              <span>Total Akhir</span>
              <span style={{ color: C.crimson, fontSize: 18 }}>Rp {currency(Math.max(0, subtotal + Math.round(subtotal * taxRate) + (orderType === "Dine-In" ? Math.round(subtotal * serviceRate) : 0) - promoDiscount))}</span>
            </div>
            <button onClick={handleCheckout} style={{ ...btn.primary, width: "100%", padding: 14, fontSize: 14, marginTop: 14, letterSpacing: 0.8, fontFamily: "inherit" }}
      ...
      {showModifiers && modifierItem && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(44,24,16,0.6)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 999 }}>
          <div style={{ background: C.cream, padding: 28, borderRadius: 18, width: 350, boxShadow: `0 20px 50px rgba(74,15,15,0.35)`, border: `1px solid ${C.goldLight}` }}>
                ...
                <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
                  {temperatureOptions.map(t => (
                    <button key={t} onClick={() => setModifierState(p => ({ ...p, temp: t }))} style={{
                      flex: 1, padding: 9, border: `1.5px solid ${modifierState.temp === t ? C.crimson : C.goldLight}`,
                      ...
                      color: modifierState.temp === t ? C.white : C.charcoal, fontWeight: "600", transition: "all 0.15s ease",
                    }}>{t}</button>
                  ))}
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, marginTop: 8 }}>
                {(modifierItem.category === "Drinks" ? drinkAddOns : foodAddOns).map(o => (
                  <button key={o} onClick={() => toggleTopping(o)} style={{
                    padding: 9, border: `1.5px solid ${modifierState.addOns.includes(o) ? C.crimson : C.goldLight}`,
                    ...
                    color: modifierState.addOns.includes(o) ? C.white : C.charcoal, fontWeight: "600", transition: "all 0.15s ease",
                  }}>{o}</button>
                ))}
              </div>
            </div>
            <div style={{ display: "flex", gap: 8, marginTop: 24 }}>
              <button onClick={addCustomizedToCart} style={{ ...btn.primary, flex: 1, padding: 11, fontSize: 13, fontFamily: "inherit", letterSpacing: 0.3 }}
      ...
      {toast && (
        <div style={{
          ...
          background: toast.type === "warning" ? `linear-gradient(135deg, #DC2626, #991B1B)` : `linear-gradient(135deg, ${C.crimsonDark}, ${C.charcoal})`,
